...
it("a greșit, b corect", function () {
  ...
  it("Mutant: rezultat_corect3 === 0 → nu intră pe inițializare", function () {
    const [a, b] = myObj.rezolvare(2, 1, 3); // a = 5, b = 2 (exemplu)
  
    Ex1.rezultat_corect3 = 0; // mutantul va considera că 0 trebuie inițializat (greșit)
    Ex1.rezultat_corect2 = 0;
    Ex1.rezultat_corect1 = 0;
  
    myObj.verificare(2, 1, 3, a, b);
  
    // Dacă mutantul este activ, rezultatul va deveni `a` (adică 5)
    // Testul pică și mutantul e omorât
    expect(Ex1.rezultat_corect3).to.equal(0);
    expect(Ex1.rezultat_corect2).to.equal(0);
    expect(Ex1.rezultat_corect1).to.equal(0);
  });
