...
describe("Testare structurala", function () {
    ...
    // it("Afiseaza 'input invalid' (b < 1)", function () {
    //     expect(myObj.rezolvare(5, 9, 10)).to.deep.equal(["input invalid", null]);
    // });
    ...
    it("Caz adevarat si caz fals - valid vs invalid -conditie C2 (d<=r)", function () {
        expect(myObj.rezolvare(4, 1, 7)).to.deep.equal([9, 2]);
        expect(myObj.rezolvare(4, 3, 3)).to.deep.equal(["input invalid", null]); // invalid
    });

    it("Caz adevarat si caz fals - valid vs invalid -conditie C3 (r<0)", function () {
        expect(myObj.rezolvare(4, 1, 7)).to.deep.equal([9, 2]);
        expect(myObj.rezolvare(4, -1, 8)).to.deep.equal(["input invalid", null]); // invalid
        expect(myObj.rezolvare(4, 1, 7)).to.deep.equal([9, 2]); // ca să intri și pe true și pe false
        expect(myObj.rezolvare(4, -1, 8)).to.deep.equal(["input invalid", null]); // negativ
    ...
    it("Caz adevarat si caz fals - valid vs invalid -conditie C5 ( d/c <= r)", function () {
        expect(myObj.rezolvare(4, 1, 7)).to.deep.equal([9, 2]);
        expect(myObj.rezolvare(3, 6, 8)).to.deep.equal(["input invalid", null]); // invalid
        expect(myObj.rezolvare(4, 1, 7)).to.deep.equal([9, 2]); // caz în care NU e <= r
    });
