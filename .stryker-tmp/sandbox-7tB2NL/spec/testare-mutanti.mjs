...
it("a greșit, b corect", function () {
  ...
  it("Mutant: rezultat_corect3 === 0 → nu intră pe inițializare", function () {
    ...
    Ex1.rezultat_corect2 = 10; // valori arbitrare ≠ rezultat
    Ex1.rezultat_corect1 = 20;
  
    const [a, b] = myObj.rezolvare(2, 1, 3);
    myObj.verificare(2, 1, 3, a, b);
  
    // Dacă intră pe inițializare, valoarea 0 va fi suprascrisă
    expect(Ex1.rezultat_corect3).to.equal(0); // testul va omorî mutantul
    expect(Ex1.rezultat_corect2).to.equal(10);
    expect(Ex1.rezultat_corect1).to.equal(20);
  });
